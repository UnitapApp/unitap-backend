...
class Command(BaseCommand):
    help = "Fixes deleted Arbitrum records"
    ...
    def handle(self, *args, **options):
        ...
        # open arb_batch.json file
        # read in the json
        # each record in the json is TransactionBatch with chain_id = 42161
        ...
        # arb_batch.json file path
        file_path = os.path.join(os.path.dirname(__file__), "arb_batch.json")

        with open(file_path, "r") as f:
            arb_batch = json.load(f)

        # sort the batches by timestamp
        arb_batch.sort(key=lambda x: x["timestamp"])

        for batch in arb_batch:
        ...
        self.stdout.write(self.style.SUCCESS("Successfully fixed Arbitrum records"))
